...
class NeuralNets(QDialog):
	...
	def loadData(self):
		...
		titles='IdClient Client DocDate Amount PayDate'.split()
		...
		self.dgvnn.setColumnCount(8)
		self.dgvnn.setHorizontalHeaderLabels(['IdClient','Client','DocDate','Amount','PayDate','NumDays','TypeCli','Class'])

		for i in range(len(self.dfcredit)):
			row = self.dfcredit.iloc[i]
			self.dgvnn.insertRow(i)
			idclient = QTableWidgetItem(str(row[0]))
			client = QTableWidgetItem(str(row[1]))
			docdate = QTableWidgetItem(str(row[2]))
			amount = QTableWidgetItem(str(row[3]))
			paydate = QTableWidgetItem(str(row[4]))
			numdays = QTableWidgetItem(str(row[5]))
			typecli = QTableWidgetItem(str(row[6]))
			classcli = QTableWidgetItem(str(row[7]))
			self.dgvnn.setItem(i, 0, idclient)
			self.dgvnn.setItem(i, 1, client)
			self.dgvnn.setItem(i, 2, docdate)
			self.dgvnn.setItem(i, 3, amount)
			self.dgvnn.setItem(i, 4, paydate)
			self.dgvnn.setItem(i, 5, numdays)
			self.dgvnn.setItem(i, 6, typecli)
			self.dgvnn.setItem(i, 7, classcli)
	...
	def Perceptron(self):
		...
		cls = Perceptron(n_iter=10000, eta0=0.0001,
	...
	def fit(self, X, y):
		...
		ax[1].plot(self.cost,lw=2, color='blue',label='CostTraining')
		ax[1].plot(self.costtest,lw=2, color='red',label='CostTest')
