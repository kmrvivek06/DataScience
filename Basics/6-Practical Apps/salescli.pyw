...
#   QDialog.__init__(self)
 ...
 def salescli(self):
    ...
    titles='Periodo Giro Cliente Total Util Rentab(%)'.split()
    ...
    self.df['Rentab(%)'] = ((self.df['Util'] / self.df['Total'])*100).apply(lambda x: round(x,2))
    ...
    self.tablecli.setHorizontalHeaderLabels(['Periodo', 'Giro', 'Cliente', 'Total', 'Util', 'Rentab(%)'])
              
    for i in range(len(self.df)):
            row = self.df.iloc[i]
            self.tablecli.insertRow(i)
            periodo = QTableWidgetItem(str(row[0]))
            giro = QTableWidgetItem(str(row[1]))
            cliente = QTableWidgetItem(str(row[2]))
            total = QTableWidgetItem(str(row[3]))
            util = QTableWidgetItem(str(row[4]))
            rentab = QTableWidgetItem(str(row[5]))
            self.tablecli.setItem(i, 0, periodo)
            self.tablecli.setItem(i, 1, giro)
            self.tablecli.setItem(i, 2, cliente)
            self.tablecli.setItem(i, 3, total)
            self.tablecli.setItem(i, 4, util)
            self.tablecli.setItem(i, 5, rentab)
            
    self.tablecli.resizeColumnsToContents()

    self.dfpivot=self.df.pivot_table(values='Total',index=['Giro','Cliente'],columns=['Periodo'])
